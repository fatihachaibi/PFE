...
export var useRowManager = function useRowManager(_ref) {
  ...
  var getRow = useCallback(function (_ref2) {
    ...
    if (rowElement == null) {
      ...
      if (rowClasses === null || rowClasses === void 0 ? void 0 : rowClasses[rowIndex]) {
      ...
      innerGridRef.current.style.position = 'relative'; // add the element to the grid

      innerGridRef.current.appendChild(rowElement); // add the element to the row map

      rowIdToElements.current.set(rowIndex, rowElement); // watch the row's children, if they all disappear then remove this row
    ...
    return rowElement;
  }, [rowClasses, innerGridRef]); // Update row classes dynamically whenever a new prop is passed in
