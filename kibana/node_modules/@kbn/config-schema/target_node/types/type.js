...
class Type {
  ...
  constructor(schema, options = {}) {
    ...
    if (options.defaultValue !== undefined) {
      schema = schema.optional(); // If default value is a function, then we must provide description for it.
...
function recursiveGetSchemaStructure(internalSchema, path = []) {
  const array = []; // Note: we are relying on Joi internals to obtain the schema structure (recursive keys).
