...
class AnalyticsClient {
  ...
  constructor(initContext) {
    ...
    (0, _defineProperty2.default)(this, "telemetryCounter$", this.internalTelemetryCounter$.pipe((0, _operators.share)()));
    ...
    (0, _defineProperty2.default)(this, "optInConfigWithReplay$", this.optInConfig$.pipe((0, _operators.filter)(optInConfig => typeof optInConfig !== 'undefined'), (0, _operators.shareReplay)(1)));
    (0, _defineProperty2.default)(this, "contextWithReplay$", this.context$.pipe((0, _operators.skipWhile)(() => !this.optInConfig$.value), // Do not forward the context events until we have an optInConfig value
    ...
    (0, _defineProperty2.default)(this, "reportEvent", (eventType, eventData) => {
      ...
      this.internalTelemetryCounter$.next({
        type: _events.TelemetryCounterType.enqueued,
        source: 'client',
      ...
      if (!this.eventTypeRegistry.get(eventType)) {
        this.internalTelemetryCounter$.next({
          type: _events.TelemetryCounterType.dropped,
          source: 'client',
          event_type: eventType,
      ...
      if (this.initContext.isDev) {// TODO: In the future we may need to validate the eventData based on the eventType's registered schema (only if isDev)
      }
    ...
    (0, _defineProperty2.default)(this, "registerShipper", (ShipperClass, shipperConfig, {
      ...
      const shipper = new ShipperClass(shipperConfig, { ...this.initContext,
      ...
      (_shipper$telemetryCou = shipper.telemetryCounter$) === null || _shipper$telemetryCou === void 0 ? void 0 : _shipper$telemetryCou.subscribe(counter => this.internalTelemetryCounter$.next({ ...counter,
    ...
    this.contextService = new _context_service.ContextService(this.context$, this.initContext.isDev);
  ...
  sendToShipper(eventType, events) {
    ...
    this.shippersRegistry.getShippersForEventType(eventType).forEach((shipper, shipperName) => {
      ...
      const isShipperOptedIn = (_this$optInConfig$$va = this.optInConfig$.value) === null || _this$optInConfig$$va === void 0 ? void 0 : _this$optInConfig$$va.isShipperOptedIn(shipperName, eventType); // Only send it to the non-explicitly opted-out shippers
    ...
    if (sentToShipper) {
      this.internalTelemetryCounter$.next({
        type: _events.TelemetryCounterType.sent_to_shipper,
        source: 'client',
        event_type: eventType,
        code: 'OK',
        count: events.length
  ...
  reportEnqueuedEventsWhenClientIsReady() {
    ...
    const configReceivedAndShipperReceivedObserver$ = (0, _rxjs.combineLatest)([this.optInConfigWithReplay$, (0, _rxjs.merge)([this.shipperRegistered$, // Merging shipperRegistered$ with the optInConfigWithReplay$ when optedIn is false, so that we don't need to wait for the shipper if opted-in === false
    this.optInConfigWithReplay$.pipe((0, _operators.filter)(cfg => (cfg === null || cfg === void 0 ? void 0 : cfg.isOptedIn()) === false))])]); // Flush the internal queue when we get any optInConfig and, at least, 1 shipper

    this.internalEventQueue$.pipe( // Take until will close the observer once we reach the condition below
    (0, _operators.takeUntil)(configReceivedAndShipperReceivedObserver$), // Accumulate the events until we can send them
    (0, _operators.buffer)(configReceivedAndShipperReceivedObserver$), // Minimal delay only to make this chain async and let the optIn operation to complete first.
    (0, _operators.delay)(0), // Re-emit the context to make sure all the shippers got it (only if opted-in)
    (0, _operators.tap)(() => {
    ...
    (0, _operators.delay)(0), // Flatten the array of events
    (0, _operators.concatMap)(events => (0, _rxjs.from)(events)), // Discard opted-out events
    (0, _operators.filter)(event => {
    ...
    (0, _operators.groupBy)(event => event.event_type), (0, _operators.mergeMap)(groupedObservable => groupedObservable.pipe((0, _operators.bufferCount)(1000), // Batching up-to 1000 events per event type for backpressure reasons
    (0, _operators.map)(events => ({
