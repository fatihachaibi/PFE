...
class ElasticsearchService {
  ...
  async setup(deps) {
    ...
    const clusterInfo$ = (0, _get_cluster_info.getClusterInfo$)(this.client.asInternalUser);
    (0, _register_analytics_context_provider.registerAnalyticsContextProvider)(deps.analytics, clusterInfo$);
    return {
      legacy: {
        config$: this.config$
      },
  ...
  async start() {
    ...
    const config = await (0, _rxjs.firstValueFrom)(this.config$); // Log every error we may encounter in the connection to Elasticsearch
