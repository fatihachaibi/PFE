...
class EnvironmentService {
  ...
  async preboot({
    ...
    const [pathConfig, serverConfig, pidConfig] = await Promise.all([(0, _rxjs.firstValueFrom)(this.configService.atPath(_utils.config.path)), (0, _rxjs.firstValueFrom)(this.configService.atPath(_http.config.path)), (0, _rxjs.firstValueFrom)(this.configService.atPath(_pid_config.config.path))]); // Log unhandled rejections so that we can fix them in preparation for https://github.com/elastic/kibana/issues/77469
