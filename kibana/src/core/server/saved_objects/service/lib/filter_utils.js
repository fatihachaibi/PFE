...
const validateConvertFilterToKueryNode = (allowedTypes, filter, indexMapping) => {
  if (filter && indexMapping) {
    let filterKueryNode = typeof filter === 'string' ? esKuery.fromKueryExpression(filter) : (0, _lodash.cloneDeep)(filter);
    ...
    validationFilterKuery.forEach(item => {
      ...
      if (item.isSavedObjectAttr) {
        ...
        existingKueryNode.arguments[0].value = keySavedObjectAttr === 'id' ? '_id' : keySavedObjectAttr;
        const itemType = allowedTypes.filter(t => t === item.type);

        if (itemType.length === 1) {
          const kueryToAdd = esKuery.nodeTypes.function.buildNode('and', [esKuery.nodeTypes.function.buildNode('is', 'type', itemType[0]), existingKueryNode]);

          if (path.length > 0) {
            (0, _saferLodashSet.set)(filterKueryNode, path, kueryToAdd);
          } else {
