...
var _find_shared_origin_objects = require("./find_shared_origin_objects");
...
const ALIAS_OR_SHARED_ORIGIN_SEARCH_PER_PAGE = 100;
/**
...
exports.ALIAS_OR_SHARED_ORIGIN_SEARCH_PER_PAGE = ALIAS_OR_SHARED_ORIGIN_SEARCH_PER_PAGE;
...
async function collectMultiNamespaceReferences(params) {
  ...
  const aliasesMap = await (0, _legacy_url_aliases.findLegacyUrlAliases)(createPointInTimeFinder, objectsToFindAliasesFor, ALIAS_OR_SHARED_ORIGIN_SEARCH_PER_PAGE);
  const objectOriginsToSearchFor = objectsWithContext.filter(({
    spaces
  }) => spaces.length !== 0).map(({
    type,
    id,
  ...
  const originsMap = await (0, _find_shared_origin_objects.findSharedOriginObjects)(createPointInTimeFinder, objectOriginsToSearchFor, ALIAS_OR_SHARED_ORIGIN_SEARCH_PER_PAGE);
  const results = objectsWithContext.map(obj => {
    const aliasesVal = aliasesMap.get((0, _internal_utils.getObjectKey)(obj));
    const spacesWithMatchingAliases = aliasesVal && Array.from(aliasesVal).sort();
    const originsVal = originsMap.get((0, _internal_utils.getObjectKey)({
      type: obj.type,
