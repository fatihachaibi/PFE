...
(function () {
  ...
  var isPreserveSymlinksPresent = nodeOptions.includes(preserveSymlinksOption) || nodeExecArgv.includes(preserveSymlinksOption);
  var isPreserveSymlinksMainPresent = nodeOptions.includes(preserveSymlinksMainOption) || nodeExecArgv.includes(preserveSymlinksMainOption);
  ...
  var getExitCodeFromSpawnResult = function (spawnResult) {
    ...
    if (spawnResult.signal !== null) {
      console.log('ensure_node_preserve_symlinks wrapper: process exitted with signal', spawnResult.signal);
      return 1;
    }

    if (spawnResult.error) {
      console.log('ensure_node_preserve_symlinks wrapper: process exitted with error', spawnResult.error);
      return 1;
    }
  ...
  var spawnResult = cp.spawnSync(nodeArgv[0], nodeArgs.concat(restArgs), {
    stdio: 'inherit'
  });
