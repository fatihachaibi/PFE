...
class Execution {
  ...
  validate(value, argDef) {
    ...
    if (argDef.strict && (_argDef$options = argDef.options) !== null && _argDef$options !== void 0 && _argDef$options.length && !argDef.options.includes(value)) {
      throw new Error(`Value '${value}' is not among the allowed options for argument '${argDef.name}': '${argDef.options.join("', '")}'`);
    }
  ...
  resolveArgs(fnDef, input, argAsts) {
    return (0, _rxjs.defer)(() => {
      ...
      }), (0, _operators.tap)(value => this.validate(value, argDefs[argName])))));
