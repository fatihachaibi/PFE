...
var _common2 = require("../../../../field_formats/common");
...
class SearchSource {
  ...
  fetch(options = {}) {
    return (0, _rxjs.lastValueFrom)(this.fetch$(options)).then(r => {
    ...
    return search({
      ...
      return new _rxjs.Observable(obs => {
        if ((0, _.isErrorResponse)(response)) {
          obs.error(response);
        } else if ((0, _.isPartialResponse)(response)) {
  ...
  flatten() {
    ...
    const metaFields = (_getConfig = getConfig(_.UI_SETTINGS.META_FIELDS)) !== null && _getConfig !== void 0 ? _getConfig : []; // get some special field types from the index pattern
    ...
    if (highlightAll && body.query) {
      body.highlight = (0, _common2.getHighlightRequest)(getConfig(_.UI_SETTINGS.DOC_HIGHLIGHT));
