...
class TabifyBuckets {
  ...
  dropPartials(agg, timeRange) {
    ...
    const interval = agg.params.used_interval ? (0, _aggs.parseInterval)(agg.serialize().params.used_interval) : _moment.default.duration(this.buckets[1].key - this.buckets[0].key);
    this.buckets = this.buckets.filter(bucket => {
      if ((0, _moment.default)(bucket.key).isBefore(timeRange.from)) {
        return false;
      }

      if ((0, _moment.default)(bucket.key).add(interval).isAfter(timeRange.to)) {
        return false;
      }
