...
class ExecuteReportTask {
  ...
  async _claimJob(task) {
    ...
    if (report.attempts >= maxAttempts) {
      const err = new _errors.QueueTimeoutError(`Max attempts reached (${maxAttempts}). Queue timeout reached.`);
  ...
  async _performJob(task, cancellationToken, stream) {
    ...
    const queueTimeout = (0, _schema_utils.durationToNumber)(this.config.queue.timeout);
    return Rx.lastValueFrom(Rx.from(runner.jobExecutor(task.id, task.payload, cancellationToken, stream)).pipe((0, _operators.timeout)(queueTimeout)) // throw an error if a value is not emitted before timeout
    );
  }
  ...
  async throwIfKibanaShutsDown() {
    ...
    throw new _errors.KibanaShuttingDownError();
  }
  /*
      ...
      return {
        ...
        run: async () => {
          ...
          try {
            ...
            const output = await Promise.race([this._performJob(task, cancellationToken, stream), this.throwIfKibanaShutsDown()]);
            stream.end();
            ...
            eventLog.logExecutionComplete({ ...((_output$metrics = output.metrics) !== null && _output$metrics !== void 0 ? _output$metrics : {}),
            ...
            if (attempts < maxAttempts) {
              ...
              try {
                ...
                error.details = error.details || `Max attempts (${attempts}) reached for job ${jobId}. Failed with: ${failedToExecuteErr.message}`;
