...
const METRIC_PERCENTILES = [50, 75, 95, 99]; // indexes some key/count buckets by the "key" property
...
function getAggStats(aggs, metrics) {
  var _get;
  ...
  const jobTypes = jobBuckets.reduce((accum, bucket) => {
    ...
    const deprecatedCount = isDeprecated === null || isDeprecated === void 0 ? void 0 : isDeprecated.doc_count; // format the search results into the telemetry schema
...
async function handleResponse(response) {
  ...
  if (rangesBuckets) {
    ...
    last7DaysUsage = last7Days ? getAggStats(last7Days) : {}; // calculate metrics per job type for the stats covering all-time

    const metrics = normalizeMetrics((_response$aggregation2 = response.aggregations) === null || _response$aggregation2 === void 0 ? void 0 : _response$aggregation2.metrics);
