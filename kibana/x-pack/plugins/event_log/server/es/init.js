...
async function initializeEsResources(esContext) {
  const steps = new EsInitializationSteps(esContext); // today, setExistingAssetsToHidden() never throws, but just in case ...
...
function getRetryOptions(esContext, operation) {
  const logger = esContext.logger; // should retry on the order of 2s, 4s, 8s, 16s
