...
function setupSavedObjects(savedObjects, config) {
  savedObjects.registerType({
    ...
    excludeOnUpgrade: async ({
      ...
      const oldestNeededActionParams = await (0, _oldest_idle_action_task.getOldestIdleActionTask)(readonlyEsClient, _constants.TASK_MANAGER_INDEX); // Delete all action tasks that have failed and are no longer needed
