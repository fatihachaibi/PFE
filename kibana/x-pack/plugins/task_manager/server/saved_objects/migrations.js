...
function getMigrations() {
  return {
    '7.4.0': executeMigrationWithErrorHandling(doc => ({ ...doc,
      ...
      return { ...doc,
        attributes: { ...doc.attributes,
          params: JSON.stringify({ ...params,
            alertId: newId // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  if (doc.attributes.taskType.startsWith('actions:')) {
    ...
    if (params.actionTaskParamsId && params.spaceId && params.spaceId !== 'default') {
      ...
      return { ...doc,
        attributes: { ...doc.attributes,
          params: JSON.stringify({ ...params,
            actionTaskParamsId: newId // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
function resetUnrecognizedStatus(doc) {
  ...
  if (status && status === 'unrecognized') {
    const taskType = doc.attributes.taskType; // If task type is in the REMOVED_TYPES list, maintain "unrecognized" status
