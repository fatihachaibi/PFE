...
const generateAlertOpts = ({
  ...
  switch (action) {
    ...
    case _plugin.EVENT_LOG_ACTIONS.activeInstance:
      message = subgroup ? `test:1: 'rule-name' active alert: '${id}' in actionGroup(subgroup): 'default(${subgroup})'` : `test:1: 'rule-name' active alert: '${id}' in actionGroup: 'default'`;
      break;
...
const generateEnqueueFunctionInput = () => ({
  apiKey: 'MTIzOmFiYw==',
