...
class AlertingEventLogger {
  ...
  start() {
    ...
    this.eventLogger.startTiming(this.event, this.startTime); // Create and log "execute-start" event
  ...
  getStartAndDuration() {
    var _this$event, _this$event$event;

    return {
      start: this.startTime,
      duration: (_this$event = this.event) === null || _this$event === void 0 ? void 0 : (_this$event$event = _this$event.event) === null || _this$event$event === void 0 ? void 0 : _this$event$event.duration
    };
  }
  ...
  logAction(action) {
    ...
    if (status) {
      ...
      if (status.error) {
        var _status$error, _this$event2, _this$event2$error;

        updateEvent(this.event, {
          outcome: 'failure',
          reason: ((_status$error = status.error) === null || _status$error === void 0 ? void 0 : _status$error.reason) || 'unknown',
          error: ((_this$event2 = this.event) === null || _this$event2 === void 0 ? void 0 : (_this$event2$error = _this$event2.error) === null || _this$event2$error === void 0 ? void 0 : _this$event2$error.message) || status.error.message,
          ...(this.event.message ? {} : {
        ...
        if (status.warning) {
          var _status$warning, _status$warning2, _this$event3;

          updateEvent(this.event, {
            reason: ((_status$warning = status.warning) === null || _status$warning === void 0 ? void 0 : _status$warning.reason) || 'unknown',
            message: ((_status$warning2 = status.warning) === null || _status$warning2 === void 0 ? void 0 : _status$warning2.message) || ((_this$event3 = this.event) === null || _this$event3 === void 0 ? void 0 : _this$event3.message)
          });
        }
      }
    }
...
function createActionExecuteRecord(context, action) {
  return (0, _create_alert_event_log_record_object.createAlertEventLogRecordObject)({
    ...
    message: `alert: ${context.ruleType.id}:${context.ruleId}: '${context.ruleName}' instanceId: '${action.alertId}' scheduled ${action.alertSubgroup ? `actionGroup(subgroup): '${action.alertGroup}(${action.alertSubgroup})'` : `actionGroup: '${action.alertGroup}'`} action: ${action.typeId}:${action.id}`,
    savedObjects: [{
      id: context.ruleId,
      type: 'alert',
      typeId: context.ruleType.id,
