  ...
  return ({
    ...
    return {
      async run() {
        ...
        return Promise.all([(0, _alerting_telemetry.getTotalCountAggregations)(esClient, kibanaIndex, logger), (0, _alerting_telemetry.getTotalCountInUse)(esClient, kibanaIndex, logger), (0, _alerting_telemetry.getExecutionsPerDayCount)(esClient, eventLogIndex, logger), (0, _alerting_telemetry.getExecutionTimeoutsPerDayCount)(esClient, eventLogIndex, logger), (0, _alerting_telemetry.getFailedAndUnrecognizedTasksPerDay)(esClient, taskManagerIndex, logger)]).then(([totalCountAggregations, totalInUse, dailyExecutionCounts, dailyExecutionTimeoutCounts, dailyFailedAndUnrecognizedTasks]) => {
          return {
            state: {
              runs: (state.runs || 0) + 1,
