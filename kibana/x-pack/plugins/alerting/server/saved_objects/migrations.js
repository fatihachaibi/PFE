...
exports.isSiemSignalsRuleType = exports.isSecuritySolutionLegacyNotification = exports.isEsQueryRuleType = exports.isDetectionEngineAADRuleType = exports.isAnyActionSupportIncidents = void 0;
...
function getMigrations(encryptedSavedObjects, searchSourceMigrations, isPreconfigured) {
  ...
  const migrationRules820 = createEsoMigration(encryptedSavedObjects, doc => true, pipeMigrations(addMappedParams));
  const migrationRules830 = createEsoMigration(encryptedSavedObjects, doc => true, pipeMigrations(addSearchType, removeInternalTags, convertSnoozes));
...
function addSearchType(doc) {
  ...
  return isEsQueryRuleType(doc) && !searchType ? { ...doc,
    attributes: { ...doc.attributes,
      params: { ...doc.attributes.params,
        searchType: 'esQuery'
      }
...
function convertSnoozes(doc) {
  ...
  return { ...doc,
    attributes: { ...(0, _lodash.omit)(doc.attributes, ['snoozeEndTime']),
...
function mapSearchSourceMigrationFunc(migrateSerializedSearchSourceFields) {
  return doc => {
    ...
    if ((0, _common.isSerializedSearchSource)(serializedSearchSource)) {
      return { ..._doc,
        attributes: { ..._doc.attributes,
          params: { ..._doc.attributes.params,
...
function getSearchSourceMigrations(encryptedSavedObjects, searchSourceMigrations) {
  ...
  for (const versionKey in searchSourceMigrations) {
    if ((0, _semver.gte)(versionKey, MINIMUM_SS_MIGRATION_VERSION)) {
      const migrateSearchSource = mapSearchSourceMigrationFunc(searchSourceMigrations[versionKey]);
      filteredMigrations[versionKey] = executeMigrationWithErrorHandling(createEsoMigration(encryptedSavedObjects, doc => isEsQueryRuleType(doc), pipeMigrations(migrateSearchSource)), versionKey);
    }
  }
