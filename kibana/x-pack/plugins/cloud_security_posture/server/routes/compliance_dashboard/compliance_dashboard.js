...
const getClustersTrends = (clustersWithoutTrends, trends) => clustersWithoutTrends.map(cluster => ({ ...cluster,
  trend: trends.map(({
    timestamp,
...
const defineGetComplianceDashboardRoute = (router, cspContext) => router.get({
  ...
  try {
    ...
    const [stats, groupedFindingsEvaluation, clustersWithoutTrends, trends] = await Promise.all([(0, _get_stats.getStats)(esClient, query, pitId), (0, _get_grouped_findings_evaluation.getGroupedFindingsEvaluation)(esClient, query, pitId), (0, _get_clusters.getClusters)(esClient, query, pitId), (0, _get_trends.getTrends)(esClient)]); // Try closing the PIT, if it fails we can safely ignore the error since it closes itself after the keep alive
