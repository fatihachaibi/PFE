...
const createLogThresholdExecutor = libs => libs.logsRules.createLifecycleRuleExecutor(async ({
  ...
  try {
    ...
    if (!(0, _log_threshold.isRatioRuleParams)(validatedParams)) {
      await executeAlert(validatedParams, timestampField, indices, runtimeMappings, scopedClusterClient.asCurrentUser, alertFactory, startedAt.valueOf());
    } else {
      await executeRatioAlert(validatedParams, timestampField, indices, runtimeMappings, scopedClusterClient.asCurrentUser, alertFactory, startedAt.valueOf());
    }
    ...
    processRecoveredAlerts(recoveredAlerts, startedAt, getAlertStartedDate, basePath, validatedParams);
  } catch (e) {
    throw new Error(e);
  }
});
...
async function executeAlert(ruleParams, timestampField, indexPattern, runtimeMappings, esClient, alertFactory, executionTimestamp) {
...
async function executeRatioAlert(ruleParams, timestampField, indexPattern, runtimeMappings, esClient, alertFactory, executionTimestamp) {
  ...
  if ((0, _log_threshold.hasGroupBy)(ruleParams)) {
    ...
    processGroupByRatioResults(numeratorGroupedResults, denominatorGroupedResults, ruleParams, alertFactory);
  } else {
    ...
    processUngroupedRatioResults(numeratorUngroupedResults, denominatorUngroupedResults, ruleParams, alertFactory);
  }
}
...
const processUngroupedResults = (results, params, alertFactory) => {
  const {
    count,
    criteria,
    timeSize,
...
const processUngroupedRatioResults = (numeratorResults, denominatorResults, params, alertFactory) => {
  const {
    count,
    criteria,
    timeSize,
...
const processGroupByResults = (results, params, alertFactory) => {
  const {
    count,
    criteria,
    timeSize,
...
const processGroupByRatioResults = (numeratorResults, denominatorResults, params, alertFactory) => {
  const {
    count,
    criteria,
    timeSize,
...
const processRecoveredAlerts = (recoveredAlerts, startedAt, getAlertStartedDate, basePath, validatedParams) => {
