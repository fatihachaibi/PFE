...
async function getLogEntryAnomalies(context, sourceId, startTime, endTime, sort, pagination, datasets) {
  ...
  } = await getCompatibleAnomaliesJobIds(infraContext.spaceId, sourceId, infraContext.mlAnomalyDetectors);
  ...
  } = await fetchLogEntryAnomalies(infraContext.mlSystem, jobIds, startTime, endTime, sort, pagination, datasets);
  ...
  const logEntryCategoriesCountJobId = (0, _log_analysis.getJobId)(infraContext.spaceId, sourceId, _log_analysis.logEntryCategoriesJobTypes[0]);
  const {
    logEntryCategoriesById
  } = await (0, _log_entry_categories_analysis.fetchLogEntryCategories)({
    infra: infraContext
  }, logEntryCategoriesCountJobId, categoryIds);
...
async function getLogEntryExamples(context, sourceId, startTime, endTime, dataset, exampleCount, resolvedLogView, callWithRequest, categoryId) {
  ...
  const infraContext = await context.infra;
  const jobId = (0, _log_analysis.getJobId)(infraContext.spaceId, sourceId, categoryId != null ? _log_analysis.logEntryCategoriesJobTypes[0] : _log_analysis.logEntryRateJobTypes[0]);
  const {
    mlJob,
    timing: {
      spans: fetchMlJobSpans
    }
  } = await (0, _common.fetchMlJob)(infraContext.mlAnomalyDetectors, jobId);
...
async function fetchLogEntryExamples(context, sourceId, indices, runtimeMappings, timestampField, tiebreakerField, startTime, endTime, dataset, exampleCount, callWithRequest, categoryId) {
  ...
  if (categoryId) {
    ...
    const {
      ...
      infra: infraContext
    }, logEntryCategoriesCountJobId, [parsedCategoryId]);
