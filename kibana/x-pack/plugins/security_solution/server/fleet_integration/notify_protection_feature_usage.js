...
async function notifyProtectionFeatureUsage(newPackagePolicy, featureUsageService, endpointMetadataService) {
  ...
  const newPolicyConfig = (_newPackagePolicy$inp4 = newPackagePolicy.inputs[0].config) === null || _newPackagePolicy$inp4 === void 0 ? void 0 : (_newPackagePolicy$inp5 = _newPackagePolicy$inp4.policy) === null || _newPackagePolicy$inp5 === void 0 ? void 0 : _newPackagePolicy$inp5.value; // function is only called on policy update, we need to fetch the current policy
  ...
  const currentPolicyConfig = currentPackagePolicy.inputs[0].config.policy.value; // ransomware is windows only
