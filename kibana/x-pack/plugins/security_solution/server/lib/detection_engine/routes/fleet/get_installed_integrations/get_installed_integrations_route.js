...
const getInstalledIntegrationsRoute = (router, logger) => {
  router.get({
    path: _constants.DETECTION_ENGINE_INSTALLED_INTEGRATIONS_URL,
    validate: {},
    options: {
    ...
    try {
      ...
      const registryPackages = await (0, _promise_pool.initPromisePool)({
        ...
        executor: async packageInfo => {
          const registryPackage = await fleet.packages.getRegistryPackage(packageInfo.package_name, packageInfo.package_version);
