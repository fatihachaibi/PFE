...
function wrapScopedClusterClient(opts) {
  ...
  return {
    asInternalUser: wrapEsClient({ ...rest,
      esClient: scopedClusterClient.asInternalUser
    }),
    asCurrentUser: wrapEsClient({ ...rest,
...
function wrapEsClient(opts) {
  ...
  const wrappedClient = esClient.child({}); // Mutating the functions we want to wrap
...
function getWrappedSearchFn(opts) {
  const originalSearch = opts.esClient.search; // A bunch of overloads to make TypeScript happy

  async function search(params, options) {
    try {
      const searchOptions = options !== null && options !== void 0 ? options : {};
      return await originalSearch.call(opts.esClient, params, { ...searchOptions,
