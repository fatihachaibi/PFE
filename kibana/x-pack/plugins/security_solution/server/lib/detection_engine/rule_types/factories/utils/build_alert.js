...
const buildAlert = (docs, completeRule, spaceId, reason, overrides) => {
  var _overrides$severityOv, _overrides$riskScoreO, _overrides$nameOverri;
  ...
  const {
    ...
    ...commonRuleParams
  } = (0, _rule_converters.commonParamsCamelToSnake)(completeRule.ruleParams);
  const ruleParamsSnakeCase = { ...commonRuleParams,
    ...(0, _rule_converters.typeSpecificCamelToSnake)(completeRule.ruleParams)
  };
  const {
    actions,
    schedule,
...
const additionalAlertFields = doc => {
  ...
  const additionalFields = { ...(thresholdResult != null ? {
