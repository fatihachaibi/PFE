...
const getTransformedHits = (results, inputIndex, startedAt, from, threshold, ruleId) => {
  ...
  return getCombinations(results.aggregations[aggParts.name].buckets, 0, aggParts.field).reduce((acc, bucket) => {
    ...
    const source = {
      ...
      threshold_result: {
        ...
        // in the set).
        from: (_Date = new Date(bucket.minTimestamp)) !== null && _Date !== void 0 ? _Date : from
      }
    };
    acc.push({
      _index: inputIndex,
...
const transformThresholdResultsToEcs = (results, inputIndex, startedAt, from, threshold, ruleId) => {
  const transformedHits = getTransformedHits(results, inputIndex, startedAt, from, threshold, ruleId);
...
const bulkCreateThresholdSignals = async params => {
  ...
  const ecsResults = transformThresholdResultsToEcs(thresholdResults, params.inputIndexPattern.join(','), params.startedAt, params.from, ruleParams.threshold, ruleParams.ruleId);
