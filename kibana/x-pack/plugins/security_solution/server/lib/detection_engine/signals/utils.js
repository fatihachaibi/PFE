...
const getExceptions = async ({
  ...
  if (lists.length > 0) {
    try {
      ...
      }) => namespaceType); // Stream the results from the Point In Time (PIT) finder into this array
...
const getCatchupTuples = ({
  ...
  let currentFrom = originalFrom; // This loop will create tuples with overlapping time ranges, the same way rule runs have overlapping time
...
exports.createPreviewReturnType = createPreviewReturnType;
