  ...
  constructor(setupContract) {
    ...
    (0, _defineProperty2.default)(this, "runTask", async (taskInstance, core) => {
      ...
      if (transforms.length !== expectedTransforms.length) {
        ...
        reinstallAttempts = attempts; // after a reinstall attempt next check sooner with exponential backoff
      ...
      const restartAttempts = { ...taskInstance.state.restartAttempts
      };
    ...
    (0, _defineProperty2.default)(this, "reinstallTransformsIfNeeded", async (pkgVersion, currentAttempts = 0) => {
      ...
      const endpointPolicies = await this.endpointAppContext.service.getEndpointMetadataService().getAllEndpointPackagePolicies(); // endpoint not being used, no need to reinstall transforms
      ...
      try {
        ...
        const {
          packageInfo,
          paths
        } = await packageClient.getRegistryPackage(_common.FLEET_ENDPOINT_PACKAGE, pkgVersion);
