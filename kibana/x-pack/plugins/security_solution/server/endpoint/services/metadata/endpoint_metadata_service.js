...
const isAgentPolicyWithPackagePolicies = agentPolicy => {
  if (agentPolicy.package_policies.length === 0 || typeof agentPolicy.package_policies[0] !== 'string') {
    return true;
  }
...
class EndpointMetadataService {
  ...
  _endpointPackagePolicy) {
    ...
    let fleetAgentId = endpointMetadata.elastic.agent.id; // casting below is done only to remove `immutable<>` from the object if they are defined as such
