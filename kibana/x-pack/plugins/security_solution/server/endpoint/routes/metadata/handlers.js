...
const getMetadataRequestHandler = function (endpointAppContext, logger) {
  return async (context, request, response) => {
    ...
    try {
      ...
      return response.ok({
        body: await endpointMetadataService.getEnrichedHostMetadata(esClient.asInternalUser, endpointAppContext.service.getScopedFleetServices(request), request.params.id)
      });
    } catch (error) {
      return (0, _error_handler.errorHandler)(logger, response, error);
    }
...
async function enrichHostMetadata(hostMetadata, metadataRequestContext) {
  ...
  try {
    ...
    const status = await (fleetContext === null || fleetContext === void 0 ? void 0 : fleetContext.agentClient.asCurrentUser.getAgentStatusById(elasticAgentId)); // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
  ...
  try {
    ...
    const agentPolicy = await ((_metadataRequestConte4 = metadataRequestContext.endpointAppContextService.getAgentPolicyService() // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
