  ...
  for (const hit of hits) {
    for (const supportedSchema of _supported_schemas.supportedSchemas) {
      ...
      for (const constraint of supportedSchema.constraints) {
        const fieldValue = (0, _supported_schemas.getFieldAsString)(hit._source, constraint.field); // track that all the constraints are true, if one of them is false then this schema is not valid so mark it
