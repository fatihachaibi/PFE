...
class Fetcher {
  ...
  async tree(options, isInternalRequest = false) {
    const treeParts = await Promise.all([this.retrieveAncestors(options, isInternalRequest), this.retrieveDescendants(options, isInternalRequest)]);
  ...
  async formatResponse(treeNodes, options, isInternalRequest) {
    const statsIDs = [];
  ...
  async retrieveAncestors(options, isInternalRequest) {
    const ancestors = [];
  ...
  async retrieveDescendants(options, isInternalRequest) {
    const descendants = [];
