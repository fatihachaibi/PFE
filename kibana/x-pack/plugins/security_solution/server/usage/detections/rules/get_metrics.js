...
const getRuleMetrics = async ({
  ...
  try {
    ...
    const [detectionAlertsResp, caseComments, legacyRuleActions, eventLogMetricsTypeStatus] = await Promise.all([detectionAlertsRespPromise, caseCommentsPromise, legacyRuleActionsPromise, eventLogMetricsTypeStatusPromise]); // create in-memory maps for correlation
    ...
    const alertsCounts = (0, _get_alert_id_to_count_map.getAlertIdToCountMap)(detectionAlertsResp); // correlate the rule objects to the results
    ...
    const elasticRuleObjects = rulesCorrelated.filter(hit => hit.elastic_rule === true); // calculate the rule usage
