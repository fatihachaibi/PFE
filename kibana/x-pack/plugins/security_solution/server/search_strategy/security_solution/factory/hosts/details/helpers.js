...
const getHostEndpoint = async (id, deps) => {
  ...
  try {
    ...
    const endpointData = await endpointMetadataService // Using `internalUser` ES client below due to the fact that Fleet data has been moved to
    ...
    const pendingActions = fleetAgentId ? (0, _services.getPendingActionCounts)(esClient.asInternalUser, endpointMetadataService, [fleetAgentId], endpointContext.experimentalFeatures.pendingActionResponsesWithAck).then(results => {
