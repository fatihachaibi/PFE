...
const authentications = {
  ...
  parse: async (options, response) => {
    ...
    const totalCount = (0, _fp.getOr)(0, 'aggregations.stack_by_count.value', response.rawResponse);
    const fakeTotalCount = fakePossibleCount <= totalCount ? fakePossibleCount : totalCount;
    const hits = (0, _helpers.getHits)(response);
    const authenticationEdges = hits.map(hit => (0, _helpers.formatAuthenticationData)(hit));
