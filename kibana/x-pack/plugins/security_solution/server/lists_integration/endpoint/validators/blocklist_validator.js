...
const WindowsEntrySchema = _configSchema.schema.oneOf([WindowsSignerEntrySchema, _configSchema.schema.object({ ...CommonEntrySchema,
  field: _configSchema.schema.oneOf([FileHashField, FilePath])
})]);
...
const hashEntriesValidation = entries => {
  const currentHashes = entries.map(entry => entry.field); // If there are more hashes than allowed (three) then return an error
  ...
  const hashesCount = {};
  const invalidHash = []; // Check hash entries individually
