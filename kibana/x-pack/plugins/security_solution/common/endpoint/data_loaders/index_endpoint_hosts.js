  ...
  for (let j = 0; j < numDocs; j++) {
    ...
    if (enrollFleet) {
      ...
      if (!realPolicies[appliedPolicyId]) {
        ...
        (0, _utils.mergeAndAppendArrays)(response, createdPolicies); // eslint-disable-next-line require-atomic-updates
...
const deleteIndexedEndpointHosts = async (esClient, kbnClient, indexedData) => {
  ...
  if (indexedData.hosts.length) {
    ...
    response.hosts = await esClient.deleteByQuery({
      ...
      body
    }).catch(_utils.wrapErrorAndRejectPromise); // Delete from the transform destination index
