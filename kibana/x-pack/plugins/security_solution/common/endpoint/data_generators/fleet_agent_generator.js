...
class FleetAgentGenerator extends _base_data_generator.BaseDataGenerator {
  ...
  generate(overrides = {}) {
    ...
    const hit = this.generateEsHit(); // The mapping below is identical to `searchHitToAgent()` located in
    // `x-pack/plugins/fleet/server/services/agents/helpers.ts:19`

    return (0, _lodash.merge)({ // Casting here is needed because several of the attributes in `FleetServerAgent` are
