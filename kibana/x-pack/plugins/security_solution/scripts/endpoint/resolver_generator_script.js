...
function handleErr(err) {
  if (err instanceof _elasticsearch.errors.ResponseError && err.statusCode !== 404) {
...
async function addUser(esClient, user) {
  if (!user) {
    return;
  }

  const path = `_security/user/${user.username}`; // add user if doesn't exist already
...
async function deleteUser(esClient, username) {
...
async function main() {
  const argv = _yargs.default.help().options({
    ...
    withNewUser: {
      alias: 'wnu',
      describe: 'If the --fleet flag is enabled, using `--withNewUser=username:password` would add a new user with \
  ...
  let kbnClientOptions = { ...toolingLogOptions,
    url: argv.kibana
  };
  ...
  if (argv.ssl) {
    ...
    kbnClientOptions = { ...kbnClientOptions,
      url,
  ...
  let client = new _elasticsearch.Client(clientOptions);
  let user; // if fleet flag is used

  if (argv.fleet) {
    // add endpoint user if --withNewUser flag has values as username:password
    const newUserCreds = argv.withNewUser.indexOf(':') !== -1 ? argv.withNewUser.split(':') : undefined;
    user = await addUser(client, newUserCreds ? {
      username: newUserCreds[0],
      password: newUserCreds[1]
    } : undefined); // update client and kibana options before instantiating
    ...
    if (user) {
      ...
      client = new _elasticsearch.Client({ ...clientOptions,
        node
      });
    }
  ...
  const kbnClient = new _test.KbnClient({ ...kbnClientOptions
  });
  ...
  if (argv.fleet && !argv.withNewUser) {
    ...
    console.log('Please use the --withNewUser=username:password flag to add a custom user with required roles when --fleet is enabled!'); // eslint-disable-next-line no-process-exit
