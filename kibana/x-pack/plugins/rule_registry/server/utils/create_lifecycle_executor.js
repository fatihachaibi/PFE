...
const createLifecycleExecutor = (logger, ruleDataClient) => wrappedExecutor => async options => {
  ...
  if (trackedAlertStates.length) {
    ...
    result.forEach(hit => {
      const alertId = hit._source ? hit._source[_technical_rule_data_field_names.ALERT_INSTANCE_ID] : void 0;
  ...
  const makeEventsDataMapFor = alertIds => alertIds.map(alertId => {
    var _state$trackedAlerts$3, _alertData$fields$ALE;
    ...
    const {
      alertUuid,
      started
    } = (_state$trackedAlerts$3 = state.trackedAlerts[alertId]) !== null && _state$trackedAlerts$3 !== void 0 ? _state$trackedAlerts$3 : {
  ...
  const allEventsToIndex = [...trackedEventsToIndex, ...newEventsToIndex]; // Only write alerts if:
  // - writing is enabled
  //   AND
