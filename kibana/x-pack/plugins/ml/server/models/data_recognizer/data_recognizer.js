...
class DataRecognizer {
   ...
   */
  constructor(mlClusterClient, mlClient, savedObjectsClient, dataViewsService, mlSavedObjectService, request) {
  ...
  async _saveJobs(jobs, applyToAllSpaces = false) {
    ...
    if (applyToAllSpaces === true) {
      ...
      if (canCreateGlobalJobs === true) {
        await this._mlSavedObjectService.updateJobsSpaces('anomaly-detector', jobs.map(j => j.id), ['*'], // spacesToAdd
        [] // spacesToRemove
        );
      }
    }
...
function dataRecognizerFactory(client, mlClient, savedObjectsClient, dataViewsService, mlSavedObjectService, request) {
  return new DataRecognizer(client, mlClient, savedObjectsClient, dataViewsService, mlSavedObjectService, request);
}