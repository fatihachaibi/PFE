...
class AnalyticsManager {
  ...
  findJobModel(analyticsId, analyticsCreateTime) {
    return this._trainedModels.find(model => {
      ...
      return (// @ts-expect-error @elastic-elasticsearch Data frame types incomplete
        ((_model$metadata = model.metadata) === null || _model$metadata === void 0 ? void 0 : (_model$metadata$analy = _model$metadata.analytics_config) === null || _model$metadata$analy === void 0 ? void 0 : _model$metadata$analy.id) === analyticsId && // @ts-expect-error @elastic-elasticsearch Data frame types incomplete
  ...
  async getInitialElementsModelRoot(modelId) {
    ...
    const modelElements = [];
    const details = {};
    let data; // fetch model data and create model elements
  ...
  async getAnalyticsMap({
    ...
    try {
      await this.initData(); // Create first node for incoming analyticsId or modelId
      ...
      if ((0, _types.isCompleteInitialReturnType)(initialData)) {
        ...
        while (complete === false) {
          ...
          if ((0, _types.isIndexPatternLinkReturnType)(link) && link.isIndexPattern === true) {
            ...
            nextType = _data_frame_analytics.JOB_MAP_NODE_TYPES.INDEX; // Get trained model for analytics job and create model node
  ...
  async extendAnalyticsMapForAnalyticsJob({
    ...
    try {
      ...
      if (analyticsId !== undefined) {
        ...
        rootIndexNodeId = `${rootIndex}-${_data_frame_analytics.JOB_MAP_NODE_TYPES.INDEX}`; // Fetch trained model for incoming job id and add node and edge
