...
async function getValidationCheckMessages(asCurrentUser, analyzedFields, analysisConfig, source) {
  ...
  try {
    ...
    if (Array.isArray(analyzedFields) && analyzedFields.length && analyzedFields.length > _validation.INCLUDED_FIELDS_THRESHOLD) {
      ...
      if (analysisType === _data_frame_analytics.ANALYSIS_CONFIG_TYPE.OUTLIER_DETECTION && Array.isArray(analyzedFields) && analyzedFields.length < 1) {
      ...
      } else if (analysisType !== _data_frame_analytics.ANALYSIS_CONFIG_TYPE.OUTLIER_DETECTION && Array.isArray(analyzedFields) && analyzedFields.length < 2) {
