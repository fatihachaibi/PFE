...
function resultsServiceProvider(mlClient, client) {
  ...
  async function getCategoryStoppedPartitions(jobIds, fieldToBucket = _anomalies.PARTITION_FIELD_VALUE) {
    ...
    if (jobIdsWithStopOnWarnSet.length > 0) {
      ...
      const results = await mlClient.anomalySearch({
        size: 0,
        body: {
          query: {
            bool: {
  ...
  async function getDatafeedResultsChartData({
    ...
    var _datafeedConfig$indic, _bucketResp$buckets, _modelSnapshotsResp$m;
    ...
    const [datafeedConfig, jobsResponse] = await Promise.all([getDatafeedByJobId(jobId), mlClient.getJobs({
      job_id: jobId
    })]);
    ...
    const [bucketResp, annotationResp, modelSnapshotsResp] = await Promise.all([mlClient.getBuckets({
      job_id: jobId,
      body: {
        desc: true,
        start: String(start),
    ...
    const bucketResults = (_bucketResp$buckets = bucketResp === null || bucketResp === void 0 ? void 0 : bucketResp.buckets) !== null && _bucketResp$buckets !== void 0 ? _bucketResp$buckets : [];
  ...
  } = (0, _anomaly_charts.anomalyChartsDataProvider)(mlClient, client);
  return {
