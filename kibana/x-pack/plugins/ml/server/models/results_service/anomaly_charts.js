...
function anomalyChartsDataProvider(mlClient, client) {
  ...
  async function fetchMetricData(index, entityFields, query, metricFunction, // ES aggregation name
  metricFieldName, summaryCountFieldName, timeFieldName, earliestMs, latestMs, intervalMs, datafeedConfig) {
    ...
    const aggFields = (0, _datafeed_utils.getDatafeedAggregations)(datafeedConfig); // Build the criteria to use in the bool filter part of the request.
    ...
    if (metricFieldName !== undefined && metricFieldName !== '' && metricFunction) {
      ...
      if ((0, _common.isPopulatedObject)(aggFields)) {
        var _aggFields$accessor$a; // first item under aggregations can be any name, not necessarily 'buckets'


        const accessor = Object.keys(aggFields)[0];
        const tempAggs = { ...((_aggFields$accessor$a = aggFields[accessor].aggs) !== null && _aggFields$accessor$a !== void 0 ? _aggFields$accessor$a : aggFields[accessor].aggregations)
        };
      ...
      if (summaryCountFieldName !== undefined && metricFunction === _aggregation_types.ES_AGGREGATION.CARDINALITY) {
        ...
        if (typeof aggFields === 'object' && Object.keys(aggFields).length > 0) {
          var _aggFields$accessor$a2; // first item under aggregations can be any name, not necessarily 'buckets'


          const accessor = Object.keys(aggFields)[0];
          const tempAggs = { ...((_aggFields$accessor$a2 = aggFields[accessor].aggs) !== null && _aggFields$accessor$a2 !== void 0 ? _aggFields$accessor$a2 : aggFields[accessor].aggregations)
          };
  ...
  function processRecordsForDisplay(combinedJobRecords, anomalyRecords) {
    ...
    (0, _lodash.each)(anomalyRecords, record => {
      ...
      const job = combinedJobRecords[record.job_id]; // if we already know this job has datafeed aggregations we cannot support
    ...
    let recordsForSeries = []; // Convert to an array of the records with the highest record_score per unique series.
  ...
  function calculateChartRange(seriesConfigs, selectedEarliestMs, selectedLatestMs, recordsToPlot, timeFieldName, optimumNumPoints, timeBounds) {
    let tooManyBuckets = false; // Calculate the time range for the charts.
    ...
    const pointsToPlotFullSelection = Math.ceil((selectedLatestMs - selectedEarliestMs) / minBucketSpanMs); // Increase actual number of points if we can't plot the selected range
  ...
  async function getAnomalyData(combinedJobRecords, anomalyRecords, selectedEarliestMs, selectedLatestMs, numberOfPoints, timeBounds, severity = 0, maxSeries = 6) {
    ...
    data.tooManyBuckets = tooManyBuckets; // first load and wait for required data,
    ...
    const seriesPromises = []; // Use seriesConfigs list without geo data config so indices match up after seriesPromises are resolved
    ...
    function processChartData(responses, seriesIndex) {
      ...
      const chartType = (0, _chart_utils.getChartType)(seriesConfigsForPromises[seriesIndex]); // Sort records in ascending time order matching up with chart data
    ...
    const seriesToPlot = response // Don't show the charts if there was an issue retrieving metric or anomaly data
    .filter(r => {
      var _r$, _r$2;

      return ((_r$ = r[0]) === null || _r$ === void 0 ? void 0 : _r$.success) === true && ((_r$2 = r[1]) === null || _r$2 === void 0 ? void 0 : _r$2.success) === true;
    }).map((d, i) => {
      return { ...seriesConfigsForPromises[i],
        loading: false,
  ...
  async function getMetricData(config, range, job) {
    const {
      ...
      entityFields,
      bucketSpanSeconds
    } = config; // If the job uses aggregation or scripted fields, and if it's a config we don't support
    ...
    if (useSourceData) {
      ...
      try {
        const resp = await getModelPlotOutput(jobId, detectorIndex, criteriaFields, range.min, range.max, bucketSpanSeconds * 1000); // Return data in format required by the explorer charts.
  ...
  async function getEventDistributionData(index, splitField, filterField, query, metricFunction, // ES aggregation name
  metricFieldName, timeFieldName, earliestMs, latestMs, intervalMs) {
    ...
    const dataByTime = (0, _lodash.get)(resp, ['aggregations', 'sample', 'byTime', 'buckets'], []); // @ts-ignore

    const data = dataByTime.reduce((d, dataForTime) => {
      const date = +dataForTime.key;
      const entities = (0, _lodash.get)(dataForTime, ['entities', 'buckets'], []); // @ts-ignore
  ...
  async function getEventDistribution(config, range) {
    ...
    let splitField;
    let filterField = null; // Define splitField and filterField based on chartType
  ...
  async function getAnomalyChartsData(options) {
    const {
      ...
      threshold,
      timeBounds
    } = options; // First fetch records that satisfy influencers query criteria
