...
async function validateBucketSpan(client, job, duration) {
  (0, _validate_job_object.validateJobObject)(job); // if there is no duration, do not run the estimate test

  if (typeof duration === 'undefined' || typeof duration.start === 'undefined' || typeof duration.end === 'undefined') {
    return Promise.resolve([]);
  }

  const messages = []; // Bucket span must be a valid interval, greater than 0,
  ...
  const bucketSpanDays = parsedBucketSpan.asDays(); // test #1: check if bucket span is higher than define threshold
  ...
  try {
    const estimations = estimatorConfigs.map(data => {
      return new Promise(resolve => {
        (0, _bucket_span_estimator.estimateBucketSpanFactory)(client)(data).then(resolve) // this catch gets triggered when the estimation code runs without error
    ...
    if (bucketSpans.length > 0) {
      const bucketSpan = pickBucketSpan(bucketSpans); // only trigger an info-level message if the bucket span estimator is able to come up
