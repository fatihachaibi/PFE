...
function getMlClient(client, mlSavedObjectService) {
  ...
  return {
    ...
    async getCalendars(...p) {
      ...
      const response = await mlClient.getCalendars(params, { ...options,
        meta: true
      });
      const {
        jobs: allJobs
      ...
      const calJobIds = [...new Set(response.body.calendars.map(c => c.job_ids).flat())]; // find groups by getting the cal job ids which aren't real jobs.
    ...
    async getDataFrameAnalytics(...p) {
      ...
      try {
        ...
        const response = await mlClient.getDataFrameAnalytics(params, { ...options,
          meta: true
        });
        const jobs = await mlSavedObjectService.filterJobsForSpace('data-frame-analytics', // @ts-expect-error @elastic-elasticsearch Data frame types incomplete
    ...
    async getDataFrameAnalyticsStats(...p) {
      ...
      try {
        ...
        const response = await mlClient.getDataFrameAnalyticsStats(params, { ...options,
          meta: true
        });
    ...
    async getDatafeedStats(...p) {
      ...
      try {
        ...
        const response = await mlClient.getDatafeedStats(params, { ...options,
          meta: true
        });
    ...
    async getDatafeeds(...p) {
      ...
      try {
        ...
        const response = await mlClient.getDatafeeds(params, { ...options,
          meta: true
        });
        const datafeeds = await mlSavedObjectService.filterDatafeedsForSpace('anomaly-detector', response.body.datafeeds, 'datafeed_id');
        const enhancedBody = { ...response.body,
    ...
    async getJobStats(...p) {
      try {
        ...
        const response = await mlClient.getJobStats(params, { ...options,
          meta: true
        });
    ...
    async getJobs(...p) {
      try {
        ...
        const response = await mlClient.getJobs(params, { ...options,
          meta: true
        });
        const jobs = await mlSavedObjectService.filterJobsForSpace('anomaly-detector', response.body.jobs, 'job_id');
    ...
    async inferTrainedModel(...p) {
      await modelIdsCheck(p); // Temporary workaround for the incorrect inferTrainedModelDeployment function in the esclient
    ...
    async updateDatafeed(...p) {
      ...
      return client.asInternalUser.transport.request({
        ...
        body
      }, p[1]); // this should be reinstated once https://github.com/elastic/elasticsearch-js/issues/1601
      // is fixed
