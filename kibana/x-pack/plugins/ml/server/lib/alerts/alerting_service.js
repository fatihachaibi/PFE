...
function buildExplorerUrl(jobIds, timeRange, type, r) {
  var _timeRange$mode;
  ...
  const appState = {
    explorer: {
      mlExplorerFilter: { ...(r && isInfluencerResult ? {
          filterActive: true,
      ...
      mlExplorerSwimlane: { ...(r && highlightSwimLaneSelection ? {
...
function alertingServiceProvider(mlClient, datafeedsService, getFieldsFormatRegistry, getDataViewsService) {
  ...
  const getResultsFormatter = (resultType, useInitialScore = false, formatters) => {
    ...
    return v => {
      ...
      return {
        ...
        key: v.key,
        message: _i18n.i18n.translate('xpack.ml.alertTypes.anomalyDetectionAlertingRule.alertMessage', {
  ...
  return {
    ...
    execute: async params => {
      ...
      if (result) {
        const anomalyExplorerUrl = buildExplorerUrl(result.jobIds, {
          from: result.bucketRange.start,
          to: result.bucketRange.end
        }, params.resultType, result);
      ...
      return {
        ...
        isHealthy: true,
        context: {
          anomalyExplorerUrl: buildExplorerUrl(queryParams.jobIds, {
            from: `now-${queryParams.lookBackTimeInterval}`,
            to: 'now',
            mode: 'relative'
          }, queryParams.resultType),
          jobIds: queryParams.jobIds,
          message: _i18n.i18n.translate('xpack.ml.alertTypes.anomalyDetectionAlertingRule.recoveredMessage', {
