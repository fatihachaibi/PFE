...
function getRequestItemsProvider(resolveMlCapabilities, getClusterClient, savedObjectsClient, internalSavedObjectsClient, authorization, spaceEnabled, isMlReady, getUiSettings, getFieldsFormat, getDataViews) {
  return request => {
    ...
    const getSobSavedObjectService = client => {
      return (0, _saved_objects.mlSavedObjectServiceFactory)(savedObjectsClient, internalSavedObjectsClient, spaceEnabled, authorization, client, isMlReady);
    };
    ...
    if (request instanceof _server.KibanaRequest) {
      ...
      mlClient = (0, _ml_client.getMlClient)(scopedClient, mlSavedObjectService);
    } else {
