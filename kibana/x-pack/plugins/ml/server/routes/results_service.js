...
function resultsServiceRoutes({
  ...
  router.post({
    ...
    validate: {
      body: _results_service_schema.getAnomalyChartsSchema
    },
    options: {
      tags: ['access:ml:canGetJobs']
    }
  ...
  router.post({
    ...
    validate: {
      body: _results_service_schema.getAnomalyRecordsSchema
    },
    options: {
      tags: ['access:ml:canGetJobs']
    }
    ...
    try {
      ...
      const resp = await getRecordsForCriteria(jobIds, criteriaFields, threshold, earliestMs, latestMs, interval);
      return response.ok({
        body: resp
      });
    } catch (e) {
