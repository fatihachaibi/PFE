...
function savedObjectsRoutes({
  ...
  router.post({
    ...
    try {
      ...
      if (mlSavedObjectType === 'trained-model') {
        const body = await mlSavedObjectService.updateTrainedModelsSpaces(ids, [], // spacesToAdd
      ...
      const body = await mlSavedObjectService.updateJobsSpaces(mlSavedObjectType, ids, [], // spacesToAdd
      [currentSpaceId] // spacesToRemove
      );
      return response.ok({
        body
  ...
  router.get({
    ...
    try {
      ...
      const jobStatus = Object.entries(savedObjects).filter(([type]) => type === 'anomaly-detector' || type === 'data-frame-analytics').map(([, status]) => status).flat().filter(s => s.checks.jobExists).reduce((acc, cur) => {
        const type = cur.type;
  ...
  router.get({
    ...
    try {
      ...
      const savedObjects = (await checkStatus()).savedObjects;
      const modelStatus = savedObjects['trained-model'].filter(s => s.checks.trainedModelExists).reduce((acc, cur) => {
