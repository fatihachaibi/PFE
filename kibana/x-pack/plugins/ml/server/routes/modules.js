...
function recognize(client, mlClient, savedObjectsClient, dataViewsService, mlSavedObjectService, request, indexPatternTitle) {
  const dr = (0, _data_recognizer.dataRecognizerFactory)(client, mlClient, savedObjectsClient, dataViewsService, mlSavedObjectService, request);
  return dr.findMatches(indexPatternTitle);
}

function getModule(client, mlClient, savedObjectsClient, dataViewsService, mlSavedObjectService, request, moduleId) {
  const dr = (0, _data_recognizer.dataRecognizerFactory)(client, mlClient, savedObjectsClient, dataViewsService, mlSavedObjectService, request);
...
function setup(client, mlClient, savedObjectsClient, dataViewsService, mlSavedObjectService, request, moduleId, prefix, groups, indexPatternName, query, useDedicatedIndex, startDatafeed, start, end, jobOverrides, datafeedOverrides, estimateModelMemory, applyToAllSpaces) {
  const dr = (0, _data_recognizer.dataRecognizerFactory)(client, mlClient, savedObjectsClient, dataViewsService, mlSavedObjectService, request);
...
function dataRecognizerJobsExist(client, mlClient, savedObjectsClient, dataViewsService, mlSavedObjectService, request, moduleId) {
  const dr = (0, _data_recognizer.dataRecognizerFactory)(client, mlClient, savedObjectsClient, dataViewsService, mlSavedObjectService, request);
...
function dataRecognizer({
  ...
  router.get({
    ...
    try {
      ...
      const results = await recognize(client, mlClient, soClient, dataViewService, mlSavedObjectService, request, indexPatternTitle);
      return response.ok({
        body: results
      });
    } catch (e) {
  ...
  router.get({
    ...
    try {
      ...
      const soClient = (await context.core).savedObjects.client;
      const dataViewService = await getDataViewsService();
      const results = await getModule(client, mlClient, soClient, dataViewService, mlSavedObjectService, request, moduleId);
      return response.ok({
        body: results
      });
    } catch (e) {
  ...
  router.post({
    ...
    try {
      ...
      const soClient = (await context.core).savedObjects.client;
      const dataViewService = await getDataViewsService();
      const result = await setup(client, mlClient, soClient, dataViewService, mlSavedObjectService, request, moduleId, prefix, groups, indexPatternName, query, useDedicatedIndex, startDatafeed, start, end, jobOverrides, datafeedOverrides, estimateModelMemory, applyToAllSpaces);
      return response.ok({
        body: result
      });
    } catch (e) {
  ...
  router.get({
    ...
    try {
      ...
      const soClient = (await context.core).savedObjects.client;
      const dataViewService = await getDataViewsService();
      const result = await dataRecognizerJobsExist(client, mlClient, soClient, dataViewService, mlSavedObjectService, request, moduleId);
      return response.ok({
        body: result
      });
    } catch (e) {
