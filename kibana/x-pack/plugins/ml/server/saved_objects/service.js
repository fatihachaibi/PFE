...
function mlSavedObjectServiceFactory(savedObjectsClient, internalSavedObjectsClient, spacesEnabled, authorization, client, isMlReady) {
  ...
  async function _filterJobObjectIdsForSpace(jobType, ids, key, allowWildcards = false) {
    ...
    const jobIds = await _getIds(jobType, key); // check to see if any of the ids supplied contain a wildcard
  ...
  async function _createTrainedModel(modelId, job) {
    ...
    try {
      ...
      if (existingModelObject !== undefined) {
        var _existingModelObject$; // a saved object for this job already exists, this may be left over from a previously deleted job
    ...
    let initialNamespaces; // if a job exists for this model, ensure the initial namespaces for the model
  ...
  async function _filterModelObjectIdsForSpace(ids, key, allowWildcards = false) {
    ...
    const modelIds = await _getModelIds(key); // check to see if any of the ids supplied contain a wildcard
