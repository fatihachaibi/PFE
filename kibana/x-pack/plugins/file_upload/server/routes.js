...
function fileUploadRoutes(coreSetup, logger) {
  ...
  router.post({
    ...
    try {
      ...
      const esClient = (await context.core).elasticsearch.client; // `id` being `undefined` tells us that this is a new import due to create a new index.
      ...
      const result = await importData(esClient, id, index, settings, mappings, // @ts-expect-error
      ingestPipeline, data);
      return response.ok({
        body: result
      });
