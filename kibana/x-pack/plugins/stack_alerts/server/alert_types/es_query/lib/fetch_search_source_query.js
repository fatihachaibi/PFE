...
async function fetchSearchSourceQuery(alertId, params, latestTimestamp, services) {
  const {
    logger,
    searchSourceClient
  } = services;
  ...
  logger.debug(`search source query alert (${alertId}) query: ${JSON.stringify(searchSource.getSearchRequestBody())}`);
