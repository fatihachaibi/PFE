...
function addMessages(ruleInfo, baseContext, params, isRecoveryMessage) {
  const title = isRecoveryMessage ? RECOVERY_TITLE(ruleInfo.name, baseContext.group) : DEFAULT_TITLE(ruleInfo.name, baseContext.group);
  const window = `${params.timeWindowSize}${params.timeWindowUnit}`;
  const message = isRecoveryMessage ? RECOVERY_MESSAGE(ruleInfo.name, baseContext, window) : DEFAULT_MESSAGE(ruleInfo.name, baseContext, window);
  return { ...baseContext,
    title,
    message
  };
