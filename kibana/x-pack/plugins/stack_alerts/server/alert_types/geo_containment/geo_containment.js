...
function getActiveEntriesAndGenerateAlerts(prevLocationMap, currLocationMap, alertFactory, shapesIdsNamesMap, currIntervalEndTime) {
  ...
  allActiveEntriesMap.forEach((locationsArr, entityName) => {
    ...
    locationsArr.forEach(({
      ...
      if (shapeLocationId !== _es_query_builder.OTHER_CATEGORY) {
        alertFactory.create(alertInstanceId).scheduleActions(_alert_type.ActionGroupId, context);
      }
    });
...
const getGeoContainmentExecutor = log => async function ({
  ...
  const allActiveEntriesMap = getActiveEntriesAndGenerateAlerts(prevLocationMap, currLocationMap, services.alertFactory, shapesIdsNamesMap, currIntervalEndTime);
  return {
    shapesFilters,
    shapesIdsNamesMap,
