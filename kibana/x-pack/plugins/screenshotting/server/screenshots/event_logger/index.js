...
class EventLogger {
  ...
  startTransaction(action) {
    ...
    return ({
      ...
      this.logEvent(action, 'complete', { ...labels,
        action
      }, this.timings[action]);
    };
  }
  ...
  log(message, action, type, metricsPre = {}, transaction) {
    ...
    this.startTiming(action);
    this.logEvent(message, 'start', { ...metricsPre,
      action
    });
    return (metricData = {}) => {
      span === null || span === void 0 ? void 0 : span.end();
      this.logEvent(message, 'complete', { ...metricsPre,
        ...metricData,
        action
      }, this.timings[action]);
    };
  ...
  error(error, action) {
    ...
    const errorData = { ...fillLogData(message, {
        action
