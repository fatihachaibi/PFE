...
const openUrl = async (browser, eventLogger, timeout, index, url, context, headers) => {
  ...
  const spanEnd = eventLogger.logScreenshottingEvent('open url', _event_logger.Actions.OPEN_URL, 'wait'); // If we're moving to another page in the app, we'll want to wait for the app to tell us
  ...
  try {
    await browser.open(url, {
      ...
      waitForSelector,
      timeout
    }, kbnLogger); // Debug logging for viewport size and resizing
