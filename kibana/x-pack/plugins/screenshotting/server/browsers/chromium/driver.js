...
function getDisallowedOutgoingUrlError(interceptedUrl) {
  return new Error(`Received disallowed outgoing URL: "${interceptedUrl}". Failing the request and closing the browser.`);
}
/**
 * @internal
 */
...
class HeadlessChromiumDriver {
  ...
  registerListeners(url, customHeaders, logger) {
    ...
    const client = this.page.client(); // We have to reach into the Chrome Devtools Protocol to apply headers as using
    ...
    client.on('Fetch.requestPaused', async interceptedRequest => {
      ...
      if (this._shouldUseCustomHeaders(url, interceptedUrl)) {
        ...
        const headers = Object.entries({ ...interceptedRequest.request.headers,
