...
class HeadlessChromiumDriverFactory {
  ...
  createPage({
    ...
    return new Rx.Observable(observer => {
      ...
      logger.debug(`Chromium launch args set to: ${chromiumArgs}`); // We set the viewport width using the client-side layout info to reduce the chances of
      ...
      (async () => {
        ...
        const startMetrics = await devTools.send('Performance.getMetrics'); // Log version info for debugging / maintenance

        const versionInfo = await devTools.send('Browser.getVersion');
        logger.debug(`Browser version: ${JSON.stringify(versionInfo)}`);
        await page.emulateTimezone(browserTimezone); // Set the default timeout for all navigation methods to the openUrl timeout
        ...
        const {
          terminate$
        } = (0, _safe_child_process.safeChildProcess)(logger, childProcess); // Ensure that the browser is closed once the observable completes.
        ...
        const driver = new _driver.HeadlessChromiumDriver(this.screenshotMode, this.config, this.basePath, page); // Rx.Observable<never>: stream to interrupt page capture
  ...
  async getErrorMessage(message) {
    for (const arg of message.args()) {
      const errorMessage = await arg.executionContext().evaluate(_arg => {
  ...
  diagnose(overrideFlags = []) {
    ...
    const finalArgs = (0, _lodash.uniq)([...DEFAULT_ARGS, ...kbnArgs, ...overrideFlags]); // On non-windows platforms, `detached: true` makes child process a
