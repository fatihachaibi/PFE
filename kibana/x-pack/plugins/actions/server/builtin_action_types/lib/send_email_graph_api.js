...
async function sendEmailGraphApi(sendEmailOptions, logger, configurationUtilities, axiosInstance) {
  ...
  const {
    ...
    messageHTML,
    graphApiUrl
  } = sendEmailOptions; // Create a new axios instance if one is not provided

  axiosInstance = (_axiosInstance = axiosInstance) !== null && _axiosInstance !== void 0 ? _axiosInstance : _axios.default.create(); // POST /users/{id | userPrincipalName}/sendMail
