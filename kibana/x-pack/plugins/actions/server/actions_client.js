...
var _is_conector_deprecated = require("./lib/is_conector_deprecated");

var _get_oauth_jwt_access_token = require("./builtin_action_types/lib/get_oauth_jwt_access_token");

var _get_oauth_client_credentials_access_token = require("./builtin_action_types/lib/get_oauth_client_credentials_access_token");
/*
...
class ActionsClient {
  ...
  async create({
    ...
    return {
      ...
      isDeprecated: (0, _is_conector_deprecated.isConnectorDeprecated)(result.attributes)
    };
  }
  /**
   * Update action
  ...
  async get({
    ...
    if (preconfiguredActionsList !== undefined) {
      ...
      return {
        ...
        isDeprecated: (0, _is_conector_deprecated.isConnectorDeprecated)(preconfiguredActionsList)
      };
    }
  ...
  async getAll() {
    ...
    const savedObjectsActions = (await this.unsecuredSavedObjectsClient.find({
      ...
      type: 'action'
    })).saved_objects.map(rawAction => actionFromSavedObject(rawAction, (0, _is_conector_deprecated.isConnectorDeprecated)(rawAction.attributes)));
    ...
    const mergedResult = [...savedObjectsActions, ...this.preconfiguredActions.map(preconfiguredAction => ({
      ...
      isPreconfigured: true,
      isDeprecated: (0, _is_conector_deprecated.isConnectorDeprecated)(preconfiguredAction)
  ...
  async getBulk(ids) {
    ...
    for (const action of bulkGetResult.saved_objects) {
      ...
      actionResults.push(actionFromSavedObject(action, (0, _is_conector_deprecated.isConnectorDeprecated)(action.attributes)));
    }
  ...
  async getOAuthAccessToken({
    ...
    await this.authorization.ensureAuthorized('update'); // Verify that token url is allowed by allowed hosts config
    ...
    const parsedUrl = _url.default.parse(options.tokenUrl, false
    /* parseQueryString */
    , true
    /* slashesDenoteHost */
    );
