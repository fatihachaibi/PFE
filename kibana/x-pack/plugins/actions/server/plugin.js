  ...
  setup(core, plugins) {
    ...
    for (const preconfiguredId of Object.keys(this.actionsConfig.preconfigured)) {
      if (preconfiguredId !== _common.AlertHistoryEsIndexConnectorId) {
        const rawPreconfiguredConnector = { ...this.actionsConfig.preconfigured[preconfiguredId],
          id: preconfiguredId,
          isPreconfigured: true
        };
        this.preconfiguredActions.push({ ...rawPreconfiguredConnector,
          isDeprecated: (0, _is_conector_deprecated.isConnectorDeprecated)(rawPreconfiguredConnector)
        });
      } else {
