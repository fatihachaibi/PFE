  ...
  return ({
    ...
    return {
      async run() {
        ...
        return Promise.all([(0, _actions_telemetry.getTotalCount)(esClient, kibanaIndex, logger, preconfiguredActions), (0, _actions_telemetry.getInUseTotalCount)(esClient, kibanaIndex, logger, undefined, preconfiguredActions), (0, _actions_telemetry.getExecutionsPerDayCount)(esClient, eventLogIndex, logger)]).then(([totalAggegations, totalInUse, totalExecutionsPerDay]) => {
          return {
            state: {
              runs: (state.runs || 0) + 1,
