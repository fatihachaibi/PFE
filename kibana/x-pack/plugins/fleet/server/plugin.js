...
class FleetPlugin {
  ...
  start(core, plugins) {
    _services.appContextService.start({
      ...
      config$: this.config$,
      experimentalFeatures: (0, _common.parseExperimentalConfigValue)(this.configInitialValue.enableExperimental || []),
    ...
    const fleetSetupPromise = (async () => {
      try {
        ...
        await plugins.licensing.license$.pipe((0, _operators.take)(1)).toPromise();
