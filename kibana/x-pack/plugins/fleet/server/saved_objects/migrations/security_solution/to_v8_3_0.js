...
const migratePackagePolicyToV830 = packagePolicyDoc => {
  ...
  if (input && input.config) {
    ...
    policy.windows.advanced = policy.windows.advanced ? { ...policy.windows.advanced,
      ...migratedPolicy
    } : { ...migratedPolicy
    };
    policy.mac.advanced = policy.mac.advanced ? { ...policy.mac.advanced,
      ...migratedPolicy
    } : { ...migratedPolicy
    };
    policy.linux.advanced = policy.linux.advanced ? { ...policy.linux.advanced,
      ...migratedPolicy
    } : { ...migratedPolicy
    };
  }
