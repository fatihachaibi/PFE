...
class AgentPolicyService {
  ...
  async _update(soClient, esClient, id, agentPolicy, user, options = {
    ...
    if (existingAgentPolicy.status === _common.agentPolicyStatuses.Inactive && agentPolicy.status !== _common.agentPolicyStatuses.Active) {
  ...
  hasAPMIntegration(agentPolicy) {
    return agentPolicy.package_policies.some(p => {
      var _p$package;

      return typeof p !== 'string' && ((_p$package = p.package) === null || _p$package === void 0 ? void 0 : _p$package.name) === _common.FLEET_APM_PACKAGE;
    });
  }
