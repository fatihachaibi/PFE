...
async function installPackageFromRegistry({
  ...
  const {
    pkgName,
    pkgVersion
  } = Registry.splitPkgKey(pkgkey); // Workaround apm issue with async spans: https://github.com/elastic/apm-agent-nodejs/issues/2611

  await Promise.resolve();

  const span = _elasticApmNode.default.startSpan(`Install package from registry ${pkgName}@${pkgVersion}`, 'package'); // if an error happens during getInstallType, report that we don't know
  ...
  try {
    ...
    const [latestPackage, {
      ...
      ignoreConstraints
    }), Registry.getRegistryPackage(pkgName, pkgVersion)]); // let the user install if using the force flag or needing to reinstall or install a previous version due to failed update
...
const saveKibanaAssetsRefs = async (savedObjectsClient, pkgName, kibanaAssets) => {
  const assetRefs = Object.values(kibanaAssets).flat().map(_install.toAssetReference); // Because Kibana assets are installed in parallel with ES assets with refresh: false, we almost always run into an
  ...
  (0, _pRetry.default)(() => savedObjectsClient.update(_constants2.PACKAGES_SAVED_OBJECT_TYPE, pkgName, {
...
const updateEsAssetReferences = async (savedObjectsClient, pkgName, currentAssets, {
  ...
  // issue that causes a conflict error due to this issue: https://github.com/elastic/kibana/issues/126240. This is safe
  // to retry constantly until it succeeds to optimize this critical user journey path as much as possible.
  await (0, _pRetry.default)(() => savedObjectsClient.update(_constants2.PACKAGES_SAVED_OBJECT_TYPE, pkgName, {
