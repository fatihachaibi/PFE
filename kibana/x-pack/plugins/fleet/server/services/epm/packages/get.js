...
async function getPackageInfo({
  ...
  const resolvedPkgVersion = pkgVersion !== '' ? pkgVersion : (_savedObject$attribut = savedObject === null || savedObject === void 0 ? void 0 : savedObject.attributes.install_version) !== null && _savedObject$attribut !== void 0 ? _savedObject$attribut : latestPackage.version; // If same version is available in registry and skipArchive is true, use the info from the registry (faster),
  // otherwise build it from the archive

  let paths;
  let packageInfo = skipArchive ? await Registry.fetchInfo(pkgName, pkgVersion).catch(() => undefined) : undefined;

  if (packageInfo) {
    var _packageInfo$assets$m, _packageInfo$assets; // Fix the paths
