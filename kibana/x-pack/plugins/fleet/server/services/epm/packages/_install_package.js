...
async function _installPackage({
  ...
  try {
    var _installedPkg$attribu; // if some installation already exists
    ...
    kibanaAssetPromise.catch(() => {}); // Use a shared array that is updated by each operation. This allows each operation to accurately update the
    // installation object with it's references without requiring a refresh of the SO index on each update (faster).

    let esReferences = (_installedPkg$attribu = installedPkg === null || installedPkg === void 0 ? void 0 : installedPkg.attributes.installed_es) !== null && _installedPkg$attribu !== void 0 ? _installedPkg$attribu : []; // the rest of the installation must happen in sequential order
    ...
    ({
      esReferences
    } = await (0, _utils.withPackageSpan)('Install Data Stream ILM policies', () => (0, _install5.installIlmForDataStream)(packageInfo, paths, esClient, savedObjectsClient, logger, esReferences))); // installs ml models
    ...
    const preparedIndexTemplates = (0, _install.prepareToInstallTemplates)(packageInfo, paths, esReferences); // Update the references for the templates and ingest pipelines together. Need to be done togther to avoid race
    ...
    const [installedTemplates] = await Promise.all([(0, _utils.withPackageSpan)('Install index templates', () => preparedIndexTemplates.install(esClient, logger)), // installs versionized pipelines without removing currently installed ones
    ...
    ({
      esReferences
    } = await (0, _utils.withPackageSpan)('Install transforms', () => (0, _install4.installTransform)(packageInfo, paths, esClient, savedObjectsClient, logger, esReferences))); // If this is an update or retrying an update, delete the previous version's pipelines
    ...
    const updatedPackage = await (0, _utils.withPackageSpan)('Update install status', () => savedObjectsClient.update(_constants.PACKAGES_SAVED_OBJECT_TYPE, pkgName, {
      version: pkgVersion,
      install_version: pkgVersion,
