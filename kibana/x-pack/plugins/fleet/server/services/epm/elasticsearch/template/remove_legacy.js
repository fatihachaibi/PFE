...
const getComponentTemplateWithSuffix = (dataStream, suffix) => {
  const baseName = (0, _.getRegistryDataStreamAssetBaseName)(dataStream);
...
const _getLegacyComponentTemplatesForPackage = (componentTemplates, installablePackage) => {
  ...
  const legacyNamesLookup = new Set(); // fill a map with all possible @mappings and @settings component
...
const removeLegacyTemplates = async params => {
  ...
  if (!legacyComponentTemplateNames.length) return; // all index templates that are composed of at least one component template
