...
async function createSetupSideEffects(soClient, esClient) {
  ...
  await Promise.all([(0, _outputs.ensurePreconfiguredOutputs)(soClient, esClient, outputsOrUndefined !== null && outputsOrUndefined !== void 0 ? outputsOrUndefined : []), _.settingsService.settingsSetup(soClient)]);
  ...
  } = await (0, _preconfiguration.ensurePreconfiguredPackagesAndPolicies)(soClient, esClient, policies, packages, defaultOutput, _constants.DEFAULT_SPACE_ID);
...
async function ensureFleetGlobalEsAssets(soClient, esClient) {
  ...
  const globalAssetsRes = await Promise.all([(0, _install2.ensureDefaultComponentTemplates)(esClient, logger), // returns an array
  ...
  if (assetResults.some(asset => asset.isCreated)) {
    ...
    await Promise.all(installedPackages.saved_objects.map(async ({
      ...
      if (installation.install_source !== 'registry') {
        ...
        if (!matchingBundledPackage) {
          ...
          await (0, _packages.installPackage)({
            ...
            logger.error(`Bundled package needs to be manually reinstalled ${installation.name} after installing Fleet global assets: ${err.message}`);
          });
          return;
        }
      }
