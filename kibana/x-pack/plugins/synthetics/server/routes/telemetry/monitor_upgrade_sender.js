...
function sendSyncTelemetryEvents(logger, eventsTelemetry, updateEvent) {
  if (eventsTelemetry === undefined) {
    return;
  }

  try {
    eventsTelemetry.queueTelemetryEvents(_constants.MONITOR_SYNC_STATE_CHANNEL, [updateEvent]);
    eventsTelemetry.queueTelemetryEvents(_constants.MONITOR_SYNC_EVENTS_CHANNEL, [updateEvent]);
  } catch (exc) {
    logger.error(`queuing telemetry events failed ${exc}`);
  }
}
...
function formatTelemetryUpdateEvent(currentMonitor, previousMonitor, kibanaVersion, isInlineScript, errors) {
...
function formatTelemetryDeleteEvent(previousMonitor, kibanaVersion, deletedAt, isInlineScript, errors) {
  let durationSinceLastUpdated = 0;
...
function formatTelemetrySyncEvent() {}
