...
const testNowMonitorRoute = () => ({
  ...
  handler: async ({
    ...
    const errors = await syntheticsService.triggerConfigs(request, [(0, _format_configs.formatHeartbeatRequest)({
      // making it enabled, even if it's disabled in the UI
      monitor: { ...normalizedMonitor.attributes,
        enabled: true
      },
      monitorId,
      customHeartbeatId: normalizedMonitor.attributes[_runtime_types.ConfigKey.CUSTOM_HEARTBEAT_ID],
      testRunId
    })]);
