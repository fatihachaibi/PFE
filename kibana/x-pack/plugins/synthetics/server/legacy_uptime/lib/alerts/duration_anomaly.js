...
const getAnomalies = async (plugins, savedObjectsClient, params, lastCheckedAt) => {
  ...
  return await getAnomaliesTableData([(0, _lib.getMLJobId)(params.monitorId)], [], [], 'auto', params.severity, // Lookback window will be 2x Bucket time span, for uptime job, for now bucket
    ...
    if (foundAnomalies) {
      ...
      anomalies.forEach((anomaly, index) => {
        ...
        const relativeViewInAppUrl = (0, _get_monitor_url.getMonitorRouteFromMonitorId)({
          monitorId: _alerts.DURATION_ANOMALY.id + index,
          dateRangeEnd: 'now',
        ...
        alertInstance.scheduleActions(_alerts.DURATION_ANOMALY.id, {
          ...
          [_action_variables.VIEW_IN_APP_URL]: (0, _common.getViewInAppUrl)(relativeViewInAppUrl, basePath),
          ...summary
        });
      });
    }

    (0, _common.setRecoveredAlertsContext)(alertFactory);
