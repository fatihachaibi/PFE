...
const tlsAlertFactory = (_server, libs) => ({
  ...
  actionVariables: {
    context: [..._translations.tlsTranslations.actionVariables, ..._translations.commonStateTranslations],
    ...
    if (foundCerts) {
      certs.forEach(cert => {
        ...
        const alertInstance = alertWithLifecycle({
          id: `${cert.common_name}-${(_cert$issuer2 = cert.issuer) === null || _cert$issuer2 === void 0 ? void 0 : _cert$issuer2.replace(/\s/g, '_')}-${cert.sha256}`,
          fields: {
        ...
        alertInstance.scheduleActions(_alerts.TLS.id, { ...summary
        });
      });
    }
