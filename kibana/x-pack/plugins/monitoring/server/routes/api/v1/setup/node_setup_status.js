...
function nodeSetupStatusRoute(server) {
  ...
  server.route({
    ...
    handler: async req => {
      ...
      const ccs = req.payload.ccs; // NOTE using try/catch because checkMonitoringAuth is expected to throw
      ...
      try {
        ...
        const indexPatterns = (0, _get_index_patterns.getIndexPatterns)(server.config, {}, ccs);
        const status = await (0, _collection.getCollectionStatus)(req, indexPatterns, undefined, nodeUuid, skipLiveData);
