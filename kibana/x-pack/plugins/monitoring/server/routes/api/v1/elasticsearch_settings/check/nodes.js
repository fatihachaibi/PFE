...
function nodesSettingsCheckRoute(server) {
  server.route({
    ...
    async handler(req) {
      try {
        const response = await (0, _elasticsearch_settings2.checkNodesSettings)(req); // needs to be try/catch to handle privilege error

        return _elasticsearch_settings.getElasticsearchSettingsNodesResponsePayloadRT.encode(response);
      } catch (err) {
