...
function clustersRoute(server) {
   ...
   */
  const validateBody = (0, _create_route_validation_function.createValidationFunction)(_cluster.postClustersRequestPayloadRT); // TODO switch from the LegacyServer route() method to the "new platform" route methods
  ...
  server.route({
    ...
    handler: async req => {
      ...
      try {
        ...
        const indexPatterns = (0, _get_index_patterns.getIndexPatterns)(config, {
          filebeatIndexPattern: config.ui.logs.index
        });
        const clusters = await (0, _get_clusters_from_request.getClustersFromRequest)(req, indexPatterns, {
