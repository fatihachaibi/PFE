...
run(async ({
  ...
  if (flags.run) {
    ...
    execa.sync(process.execPath, ['--preserve-symlinks', require.resolve('webpack-dev-server/bin/webpack-dev-server'), '--config', webpackConfig, ...(process.stdout.isTTY && !process.env.CI ? ['--progress'] : []), '--hide-modules', '--display-entrypoints', 'false', '--content-base', SHAREABLE_RUNTIME_SRC], options);
    return;
  }
  ...
  execa.sync(process.execPath, ['--preserve-symlinks', require.resolve('webpack/bin/webpack'), '--config', webpackConfig, '--hide-modules', ...(process.stdout.isTTY && !process.env.CI ? ['--progress'] : [])], { ...options,
    env
  });
