...
const servicesRoute = (0, _create_apm_server_route.createApmServerRoute)({
  ...
  async handler(resources) {
    ...
    const [setup, serviceGroup] = await Promise.all([(0, _setup_request.setupRequest)(resources), serviceGroupId ? (0, _get_service_group.getServiceGroup)({
      savedObjectsClient,
      serviceGroupId
...
const servicesDetailedStatisticsRoute = (0, _create_apm_server_route.createApmServerRoute)({
  ...
  params: t.type({
    query: t.intersection([// t.intersection seemingly only supports 5 arguments so let's wrap them in another intersection
    t.intersection([_default_api_types.environmentRt, _default_api_types.kueryRt, _default_api_types.rangeRt, _comparison_rt.offsetRt, _default_api_types.probabilityRt]), t.type({
...
const serviceThroughputRoute = (0, _create_apm_server_route.createApmServerRoute)({
  ...
  handler: async resources => {
    ...
    }), offset ? (0, _get_throughput.getThroughput)({ ...commonProps,
      start,
      end,
      offset
    }) : []]);
...
const serviceInfrastructureRoute = (0, _create_apm_server_route.createApmServerRoute)({
  endpoint: 'GET /internal/apm/services/{serviceName}/infrastructure_attributes_for_logs',
  params: t.type({
    path: t.type({
      serviceName: t.string
    }),
...
const sortedAndFilteredServicesRoute = (0, _create_apm_server_route.createApmServerRoute)({
  ...
  handler: async resources => {
    ...
    const savedObjectsClient = (await resources.context.core).savedObjects.client;
    const [setup, serviceGroup] = await Promise.all([(0, _setup_request.setupRequest)(resources), serviceGroupId ? (0, _get_service_group.getServiceGroup)({
      savedObjectsClient,
      serviceGroupId
    }) : Promise.resolve(null)]);
