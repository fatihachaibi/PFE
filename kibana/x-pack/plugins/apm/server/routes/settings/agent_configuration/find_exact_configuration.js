...
async function findExactConfiguration({
  ...
  const [agentConfig, configsAppliedToAgentsThroughFleet] = await Promise.all([internalClient.search('find_exact_agent_configuration', params), (0, _get_config_applied_to_agent_through_fleet.getConfigsAppliedToAgentsThroughFleet)({
    setup
  })]);
