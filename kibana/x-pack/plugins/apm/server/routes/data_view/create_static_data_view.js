...
async function getForceOverwrite({
  ...
  try {
    const existingDataView = await savedObjectsClient.get('index-pattern', _data_view_constants.APM_STATIC_DATA_VIEW_ID); // if the existing data view does not matches the new one, force an update
