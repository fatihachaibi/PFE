...
async function fetchSpanLinksDetails({
  ...
  return response.hits.hits.filter(({
    ...
    if (source.processor.event === _processor_event.ProcessorEvent.span) {
      const span = source;
...
async function getSpanLinksDetails({
  ...
  const linkedSpans = chunkedResponses.flat(); // Creates a map for all span links details found
  ...
  const spanLinksDetailsMap = linkedSpans.reduce((acc, {
    ...
    if (source.processor.event === _processor_event.ProcessorEvent.transaction) {
