...
function transformServiceMapResponses(response) {
  ...
  const nodeMap = allNodes.reduce((map, node) => {
    ...
    const serviceAnomalyStats = serviceName ? anomalies.serviceAnomalies.find(item => item.serviceName === serviceName) : undefined;
  ...
  const mappedConnections = connections.map(connection => {
    ...
    const label = sourceData[_elasticsearch_fieldnames.SERVICE_NAME] + ' to ' + (targetData[_elasticsearch_fieldnames.SERVICE_NAME] || targetData[_elasticsearch_fieldnames.SPAN_DESTINATION_SERVICE_RESOURCE]);
    return {
