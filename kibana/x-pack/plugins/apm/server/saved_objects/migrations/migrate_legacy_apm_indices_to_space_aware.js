...
async function migrateLegacyAPMIndicesToSpaceAware({
  ...
  try {
    ...
    const savedObjectAttributes = { ...legacyAPMIndices.attributes,
      isSpaceAware: true
    ...
    await repository.bulkCreate(spaces.saved_objects // Skip default space since it was already updated
    .filter(({
      id: spaceId
    }) => spaceId !== 'default').map(({
      id: spaceId
