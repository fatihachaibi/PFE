...
const convertFeaturesToIndicesArray = features => {
  return (0, _fp.flow)( // Map each feature into Indices[]
  (0, _fp.map)('indices'), // Flatten the array
  _fp.flatten, // And finally dedupe the indices
  _fp.uniq)(features);
};
...
function deserializeSnapshotDetails(snapshotDetailsEs, managedRepository, successfulSnapshots) {
  ...
  const snapshotIndicesWithoutSystemIndices = indices.filter(index => !systemIndices.includes(index)).sort(); // If an index has multiple failures, we'll want to see them grouped together.
