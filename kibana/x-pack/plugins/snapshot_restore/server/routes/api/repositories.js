...
function registerRepositoriesRoutes({
  ...
  router.get({
    ...
    const {
      client: clusterClient
    } = (await ctx.core).elasticsearch; // module repo types are available everywhere out of the box
    ...
    try {
      ...
      const pluginNamesAllNodes = Object.keys(nodes).map(key => {
        var _nodes$key$plugins; // extract plugin names
      ...
      Object.keys(_common.REPOSITORY_PLUGINS_MAP).forEach(repoTypeName => {
        if ( // check if this repository plugin is installed on every node
