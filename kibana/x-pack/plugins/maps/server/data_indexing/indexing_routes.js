...
function initIndexingRoutes({
  ...
  router.post({
    ...
    const indexPatternsService = await dataPlugin.indexPatterns.indexPatternsServiceFactory(coreContext.savedObjects.client, coreContext.elasticsearch.client.asCurrentUser, request);
    const result = await (0, _create_doc_source.createDocSource)(index, mappings, coreContext.elasticsearch.client, indexPatternsService);
  ...
  router.post({
    ...
    const coreContext = await context.core;
    const result = await (0, _index_data.writeDataToIndex)(request.body.index, request.body.data, coreContext.elasticsearch.client.asCurrentUser);
  ...
  router.get({
    ...
    const coreContext = await context.core;
    return await (0, _get_indexes_matching_pattern.getMatchingIndexes)(request.query.indexPattern, coreContext.elasticsearch.client, response, logger);
  });
  router.get({
    ...
    try {
      ...
      const coreContext = await context.core;
      const mappingsResp = await coreContext.elasticsearch.client.asCurrentUser.indices.getMapping({
