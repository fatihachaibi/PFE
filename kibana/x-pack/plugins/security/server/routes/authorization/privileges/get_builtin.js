...
function defineGetBuiltinPrivilegesRoutes({
  ...
  router.get({
    ...
    const privileges = await esClient.asCurrentUser.security.getBuiltinPrivileges(); // Exclude the `none` privilege, as it doesn't make sense as an option within the Kibana UI

    privileges.cluster = privileges.cluster.filter(privilege => privilege !== 'none');
    const indexPriviledges = Array.isArray(privileges.index) ? privileges.index : [privileges.index];
