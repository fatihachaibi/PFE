...
class Tokens {
  ...
  async refresh(existingRefreshToken) {
    try {
      ...
      this.logger.debug(`Failed to refresh access token: ${(0, _errors.getDetailedErrorMessage)(err)}`); // There are at least two common cases when refresh token request can fail:
  ...
  async invalidate({
    ...
    if (refreshToken) {
      ...
      try {
        ...
        this.logger.debug(`Failed to invalidate refresh token: ${(0, _errors.getDetailedErrorMessage)(err)}`); // When using already deleted refresh token, Elasticsearch responds with 404 and a body that
    ...
    if (accessToken) {
      ...
      try {
        ...
        this.logger.debug(`Failed to invalidate access token: ${(0, _errors.getDetailedErrorMessage)(err)}`); // When using already deleted access token, Elasticsearch responds with 404 and a body that
        // shows that no tokens were invalidated.
