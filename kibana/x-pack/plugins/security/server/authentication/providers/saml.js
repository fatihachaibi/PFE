...
class SAMLAuthenticationProvider extends _base.BaseAuthenticationProvider {
  ...
  async logout(request, state) {
    ...
    if (state !== null && state !== void 0 && state.accessToken || isIdPInitiatedSLORequest || isSPInitiatedSLOResponse) {
      try {
        ...
        const redirect = isIdPInitiatedSLORequest ? await this.performIdPInitiatedSingleLogout(request, this.realm || (state === null || state === void 0 ? void 0 : state.realm)) : state ? await this.performUserInitiatedSingleLogout(state.accessToken, state.refreshToken) : // Once Elasticsearch can consume logout response we'll be sending it here. See https://github.com/elastic/elasticsearch/issues/40901
  ...
  async loginWithSAMLResponse(request, samlResponse, relayState, state) {
    ...
    try {
      ...
      this.logger.debug(`Failed to log in with SAML response: ${(0, _errors.getDetailedErrorMessage)(err)}`); // Since we don't know upfront what realm is targeted by the Identity Provider initiated login
