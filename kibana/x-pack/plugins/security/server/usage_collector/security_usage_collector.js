...
function registerSecurityUsageCollector({
  ...
  const securityCollector = usageCollection.makeUsageCollector({
    ...
    fetch: () => {
      var _sessionExpirations$i, _sessionExpirations$i2, _sessionExpirations$l, _sessionExpirations$l2, _config$session$clean, _config$session$clean2, _config$authc$provide;
