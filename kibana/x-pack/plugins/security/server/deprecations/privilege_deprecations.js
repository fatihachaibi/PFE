...
const getPrivilegeDeprecationsService = ({
  ...
  const getKibanaRolesByFeatureId = async ({
    ...
    try {
      ...
      kibanaRoles = Object.entries(elasticsearchRoles).map(([roleName, elasticsearchRole]) => (0, _authorization.transformElasticsearchRoleToRole)(features, // @ts-expect-error `SecurityIndicesPrivileges.names` expected to be `string[]`
